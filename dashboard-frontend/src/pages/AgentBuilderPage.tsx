...
const TOOLS = [
  ...
  { id: 'generate_image', name: 'Generate Image', desc: 'Generate an AI image from a prompt', icon: '🖼️' },
];
...
const AgentBuilderPage: React.FC = () => {
  ...
  const [livePreviewMode, setLivePreviewMode] = useState<'text' | 'image'>('text');
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (livePreviewMode === 'image' && !payloadInput.startsWith('/image')) {
        payloadInput = `/image model=stabilityai/stable-diffusion-3-medium ${payloadInput}`;
      }
  ...
  const handleQuickChat = async (id: string) => {
    ...
    try {
      ...
      if (livePreviewMode === 'image' && !finalInput.startsWith('/image')) {
        finalInput = `/image model=stabilityai/stable-diffusion-3-medium ${finalInput}`;
      }
                ...
                <div className="ab-section ab-playground">
                  ...
                  <div className="ab-chat-input-wrap">
                    <textarea
                      ...
                      placeholder={livePreviewMode === 'image' ? "Describe the image to generate..." : "Type a message to test your agent..."}
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleRun(); } }}
                    />
                    <div className="ab-chat-actions" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <button 
                        className={`ab-btn ${livePreviewMode === 'image' ? 'blue' : 'gray'}`} 
                        onClick={() => setLivePreviewMode(prev => prev === 'text' ? 'image' : 'text')}
                        style={livePreviewMode === 'image' ? { background: 'linear-gradient(90deg, #ec4899, #8b5cf6)', color: 'white', border: 'none', padding: '0.5rem 0.75rem' } : { padding: '0.5rem 0.75rem' }}
                      ...
                      <div key={agent.id} className="ab-agent-card">
                        ...
                        {quickChatId === agent.id && (
                          <div className="ab-qchat">
                            <div className="ab-qchat-row" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                              <button 
                                className={`ab-btn ${livePreviewMode === 'image' ? 'blue' : 'gray'}`} 
                                onClick={() => setLivePreviewMode(prev => prev === 'text' ? 'image' : 'text')}
                                style={livePreviewMode === 'image' ? { background: 'linear-gradient(90deg, #ec4899, #8b5cf6)', color: 'white', border: 'none', padding: '0.5rem 0.75rem', height: '100%' } : { padding: '0.5rem 0.75rem', height: '100%' }}
                                title="Toggle Image Mode"
                              >
                                🖼️
                              </button>
                              <input className="ab-input" style={{ flex: 1 }} placeholder={livePreviewMode === 'image' ? "Describe image..." : "Type a message..."} value={quickChatInput} onChange={e => setQuickChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleQuickChat(agent.id)} />
